...
import { formatIsoLocal, midpoint, overlapsHour, __clearFormatterCacheForTests } from '../src/time.js';

// `time.ts`'s formatter cache is module-level, so it outlives each test. Left
// alone, a test that stubs `Intl` silently inherits whatever an earlier test
// cached for the same zone -- which today only works because every test
// happens to pick a fresh zone name. Clearing it here makes zone choice a free
// decision for the next author instead of a hidden rule.
afterEach(() => {
...
describe('formatIsoLocal', () => {
  ...
  // The formatter cache is keyed by zone, and every other test in this file
  // starts from a cleared cache and formats a single zone -- so this is the
  // only place a mis-keyed cache (zone B served zone A's formatter) shows up.
  ...
  describe('UTC-offset extraction from the platform longOffset string', () => {
    ...
    /**
     * Stubs Intl.DateTimeFormat so formatToParts returns a chosen set of parts.
     * Written as a `function` (not an arrow) because `formatterFor` calls it
     * with `new`, and arrows are not constructible.
     */
    function stubParts(parts: Array<{ type: string; value: string }>): void {
    ...
    it('throws rather than emitting a malformed timestamp for an unrecognized longOffset', () => {
      // The regression this guards: passing the raw value through produced
      // "2026-08-02T03:31:00PDT" -- a string that looks ISO-8601 but does not
      // parse, and would then be silently mis-read by every consumer.
...
describe('midpoint', () => {
  it('returns the instant halfway between two dates', () => {
    ...
    // Expressed as a literal instant, not as `(a + b) / 2` -- restating the
    // implementation's own formula would assert nothing.
